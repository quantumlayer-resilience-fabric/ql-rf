...
type InSpecProfile struct { //nolint:revive // stutter is intentional: name must distinguish from the db-layer Profile type in the same package
...
type InSpecResult struct { //nolint:revive // stutter is intentional: name is referenced as inspec.InSpecResult in orchestrator service; cannot rename without breaking external callers
